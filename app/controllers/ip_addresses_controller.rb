class IpAddressesController < ApplicationController
  ...
  def geolocation
    if @single_ip
      ...
      render json: CountrySerializer.new(location)
    elsif @ip_array
  ...
  def validate_ip_params
    ...
    if params[:ips]
      @ip_array = Utils.to_collection(params[:ips])
    end
    if @single_ip
      validate_ip_address
    elsif @ip_array
  ...
  def validate_ip_address
    if @single_ip
      unless Utils.validate_is_ip_address(@single_ip)
  ...
  def validate_ip_addresses
    @ip_array.each do |ip|
      unless Utils.validate_is_ip_address(ip)
